...
notation = [                                              // convert square id to board notation
...
function RenderBoard(){console.log('render board');
  ...
  for(var sq = 0; sq < 128; sq++){
    if(!(sq & 0x88)){
      var col = sq & 7;
      var row = sq >> 4;
      var color = (row + (col % 2)) % 2 ? 'dark' : 'light';
      
      board += '<div id="1" class="' + color + '-sq square fl" onClick="alert(this.id)"></div>';
    }
  }
    
  board += '</div>';
  document.getElementById('chess_board').innerHTML = board;
